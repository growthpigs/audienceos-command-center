 ...
 * encrypts and stores tokens, and triggers initial sync.
 *
 * SECURITY:
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.warn('[Gmail Callback] User denied authorization', { error })
    ...
    if (!code || !state) {
      console.warn('[Gmail Callback] Missing required parameters', { hasCode: !!code, hasState: !!state })
      const redirectUrl = new URL('/settings/integrations', request.url)
    ...
    try {
      ...
      console.warn('[Gmail Callback] Invalid state format')
      const redirectUrl = new URL('/settings/integrations', request.url)
    ...
    if (!userId || !timestamp) {
      console.warn('[Gmail Callback] Missing userId or timestamp in state')
      const redirectUrl = new URL('/settings/integrations', request.url)
      redirectUrl.searchParams.set('error', 'invalid_state')
      return NextResponse.redirect(redirectUrl.toString())
    }
    ...
    if (stateAge > MAX_STATE_AGE) {
      console.warn('[Gmail Callback] State parameter expired', { ageMs: stateAge, maxAgeMs: MAX_STATE_AGE })
      const redirectUrl = new URL('/settings/integrations', request.url)
    ...
    const encryptedRefreshToken = tokens.refresh_token
      ? serializeEncryptedToken(encryptToken(tokens.refresh_token) as any)
      : null
    ...
    fetch(`${appUrl}/api/v1/integrations/gmail/sync`, {
      method: 'POST',
      headers: {
      ...
      console.error('[Gmail Callback] Sync trigger failed:', err)
