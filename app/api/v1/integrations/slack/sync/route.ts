 ...
 * Manually triggers Slack channel and message sync for a user.
 *
 ...
 * {
 *   "success": false,
 *   "error": "Slack not connected for user",
 *   "timestamp": "2026-01-17T14:00:00.000Z"
 * }
 * ```
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!authHeader) {
      console.warn('[Slack Sync] Missing authorization header')
    ...
    if (authHeader !== expectedKey) {
      console.warn('[Slack Sync] Invalid authorization key')
      return NextResponse.json(
        {
          success: false,
    ...
    if (!userId || typeof userId !== 'string' || userId.trim().length === 0) {
      console.warn('[Slack Sync] Missing or invalid userId')
      return NextResponse.json(
        {
          success: false,
    ...
    console.log('[Slack Sync] Sync requested', { userId })

    // Step 3: Call SlackService to sync channels
    // Using async/await (not callbacks) - awaits the service call
    const result = await SlackService.syncChannels(userId)

    console.log('[Slack Sync] Sync completed', { userId, ...result })
