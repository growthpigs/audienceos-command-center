# Architectural Decisions – Sessions 2026-01-09 & 2026-01-17

## 2026-01-17 – Health Endpoint Made Public

**Decision:** Add `/api/health` to PUBLIC_ROUTES in middleware

**Reason:**
- Health check endpoint needs to be accessible without authentication
- Required for monitoring and uptime checks
- Used to verify database connection during development

**Implications:**
- External monitoring services can check application health
- `/api/health` now returns database status and latency
- No authentication required for this endpoint

---

## 2026-01-17 – TypeScript Types Regenerated from Supabase

**Decision:** Use MCP `generate_typescript_types` to keep types in sync with database

**Reason:**
- API routes were using `as any` type casting due to outdated types
- New cartridge and onboarding tables needed type definitions
- Single source of truth for database schema

**Implications:**
- `types/database.ts` now has 35 table definitions (+388 lines)
- Build passes with proper type checking
- API routes can use typed queries
